...
class LocalModelProviderService:
    ...
    def validate_endpoint(endpoint_url: str) -> str:
        ...
        if (
            ...
            raise LocalModelProviderConfigurationError(
                "Ollama endpoint must be an HTTP local address "
                "(localhost, 127.0.0.1, or host.docker.internal)."
            )
        if parsed.path not in {"", "/"} or parsed.username or parsed.password:
            raise LocalModelProviderConfigurationError(
                "Ollama endpoint must not include credentials or a path."
            )
    ...
    async def save(
        ...
        return await self._store.save(
            workspace_id, self.validate_endpoint(endpoint_url), model_name.strip()
        )
    ...
    async def readiness(self, workspace_id: UUID) -> LocalModelProviderReadiness:
        ...
        if configuration is None:
            return LocalModelProviderReadiness(
                mode="unconfigured", detail="Local Ollama is not configured for this workspace."
            )
        try:
            ...
            return LocalModelProviderReadiness(
                ...
                detail="Local Ollama is unavailable or returned an invalid readiness response.",
            )
        if configuration.model_name not in available:
            return LocalModelProviderReadiness(
                ...
                endpoint_url=configuration.endpoint_url,
                model_name=configuration.model_name,
                detail="Ollama is reachable, but the configured model is not installed.",
            )
        return LocalModelProviderReadiness(
            mode="ready",
            endpoint_url=configuration.endpoint_url,
            model_name=configuration.model_name,
            detail="Local Ollama is ready; no inference has been performed.",
        )
